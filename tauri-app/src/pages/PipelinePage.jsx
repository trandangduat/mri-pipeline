...
import {Panel, Button, inputCls, labelCls} from '../components/ui.jsx';
import {useApp} from '../AppContext.jsx';
import {SplitPaneForm} from '../components/SplitPaneForm.jsx';
import {RuntimeSection} from '../components/RuntimeSection.jsx';

function browseJsonFile(inputRef) {
...
export function PipelineStepsSection() {
  const {metadata, formValues, handlePipelineModeChange, setFormField, applyPresetConfig, print} = useApp();
  const presetFileInput = useRef(null);
  ...
  async function handlePresetFile(file) {
    try {
      ...
      print('Load preset failed', {error: error.message});
    }
  }
          ...
          return (
            ...
            >
              ...
              <select
                ...
                value={formValues[`stage_${stage.id}`] || ''}
...
export function StatsAtlasSection() {
  const {metadata, selectedStatsAtlases, addAtlas, removeAtlas} = useApp();
  const order = ['subcortical_volume', 'cortical_volume', 'cortical_thickness'];

  return (
    <Panel
      icon={<span className="flex h-5 w-5 items-center"><BarChartIcon /></span>}
...
function AtlasPicker({statKey, atlasKeys, selectedAtlases}) {
  const {metadata, addAtlas} = useApp();
...
function BarChartIcon() {
  return (
    <svg className="h-5 w-5 text-cursor-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M3 3v18h18" />
      <path d="M18 17V9" />
      <path d="M13 17V5" />
      <path d="M8 17v-3" />
...
export function InputOutputSection() {
  const {formValues, setFormField} = useApp();
  return (
    <Panel icon={<FolderInput className="h-5 w-5 text-cursor-primary" />} title="Input & Output" className="min-w-0">
      <div className="grid gap-6 grid-cols-2 max-[1080px]:grid-cols-1">
        <label className={labelCls}>
          Source
          <select name="inputSource" value={formValues.inputSource} onChange={(e) => setFormField('inputSource', e.target.value)} className={inputCls}>
        ...
        <label className={labelCls}>
          Input mode
          <select name="inputMode" value={formValues.inputMode} onChange={(e) => setFormField('inputMode', e.target.value)} className={inputCls}>
...
export function PipelinePage() {
  const {applyWorkspaceConfig, print, startPipeline} = useApp();
  const workspaceFileInput = useRef(null);
  ...
  async function handleWorkspaceFile(file) {
    try {
      ...
      print('Load workspace failed', {error: error.message});
    }
  }
  ...
  return (
    <>
      <div className="mb-5 flex flex-wrap items-center gap-3">
        ...
        <Button
          variant="danger"
          onClick={() => print('Stop pipeline', {ok: false, error: 'Select a running job in Jobs Monitor to stop it in a later slice.'})}
        >
          Stop Pipeline
        </Button>
      </div>
