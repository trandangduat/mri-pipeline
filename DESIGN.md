...
description: An AI-first code editor whose marketing site reads like a quietly-confident developer-tools brand with a warm-cream editorial canvas (`#f7f7f4`) instead of the typical dark IDE atmosphere. Near-black warm ink (`#26251e`) carries body and display alike — display sits at weight 400 with negative letter-spacing for a magazine feel rather than a bold tech voice. The single brand voltage is **Sea Blue** (`#0077b6`) reserved for primary CTAs and the wordmark. A signature pastel timeline palette (peach, mint, blue, lavender, gold) marks AI-action stages (Thinking / Reading / Editing / Grepping / Done) — only inside in-product timeline visualizations. Cards use minimal hairlines, no shadows, generous 80px section rhythm. CursorGothic for display/body, JetBrains Mono on every code surface (which is roughly half the page).

colors:
  primary: "#0077b6"
  primary-active: "#005f8f"
  ink: "#26251e"
  body: "#5a5852"
  body-strong: "#26251e"
  muted: "#807d72"
...
Cursor's marketing site reads as a quietly-confident developer brand that believes in editorial calm over IDE-darkness. The base canvas is **warm cream** (`{colors.canvas}` — #f7f7f4) holding warm near-black ink (`{colors.ink}` — #26251e) for body and display alike. The single brand voltage is **Sea Blue** (`{colors.primary}` — #0077b6) reserved for primary CTAs and the wordmark — used scarcely.
...
- Single CTA color: `{colors.primary}` (Sea Blue #0077b6). Used scarcely.
...
### Brand & Accent
- **Sea Blue** (`{colors.primary}` — #0077b6): Primary CTA pills, wordmark, hero accent. Used scarcely.
- **Sea Blue Active** (`{colors.primary-active}` — #005f8f): Press state.
...
- **On Primary** (`{colors.on-primary}` — #ffffff): White text on Sea Blue.
...
**`button-primary`** — The signature Sea Blue CTA. Background `{colors.primary}`, text `{colors.on-primary}`, type `{typography.button}` (14px / 500), padding 10px × 18px, height 40px, rounded `{rounded.md}` (8px).
...
**`cta-band`** — Pre-footer "Try Cursor now" band. Background `{colors.canvas}`, centered display headline in `{typography.display-lg}`, single Sea Blue CTA. 96px vertical padding.
...
### Do
- Reserve `{colors.primary}` (Sea Blue) for primary CTAs and brand wordmark.
...
### Don't
- Don't introduce a secondary brand action color. Sea Blue is the only one.
