        ...
        with RemoteSSHClient(self.config.ssh, lambda _line: None) as ssh:
            ...
            script = "\n".join(
                [
                    ...
                    "                if input_file:",
                    "                    image_states[input_file] = 'running' if kind == 'image_start' else ('success' if bool(event.get('success')) else 'failed')",
                    "    except Exception:",
                    "        pass",
                    "    input_files = cfg.get('input_files') or ([cfg.get('input_file')] if cfg.get('input_file') else [])",
                    "    total = max(len(input_files), event_total, len(image_states))",
                    "    success = sum(value == 'success' for value in image_states.values())",
                    "    failed = sum(value == 'failed' for value in image_states.values())",
                    "    running = sum(value == 'running' for value in image_states.values())",
                    "    batch_summary = {'total': total, 'success': success, 'failed': failed, 'running': running, 'pending': max(0, total - success - failed - running)}",
