...
export function Panel({icon, title, children, className = '', titleRight}: PanelProps) {
  return (
    <section className={`rounded-xl border border-cursor-hairline bg-white p-4 ${className}`}>
...
export function EmptyBox({message}: {message: string}) {
  return (
    <div className="mt-3 whitespace-pre-wrap rounded-lg border border-cursor-hairline bg-white p-3 text-xs text-cursor-body">
      {message}
    </div>
  );
}
