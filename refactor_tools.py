...
for line in lines:
    ...
    if in_init:
        # Check if line is an initialization of a tools_ variable
        if "self." in line and "=" in line and any(var in line for var in [
            "self.tools_tab =", "self.tools_table_frame =", "self.tools_log_text =", 
            "self.tools_log_body =", "self.tools_log_toggle_text =", "self.tools_log_visible =",
            "self.tools_checked_tools =", "self.tools_check_vars =", "self.tools_status_icon_labels =",
            "self.python_env_check_button =", "self.python_env_install_button =", "self.tools_refresh_button =",
            "self.tools_select_all_button =", "self.tools_unselect_all_button =", "self.tools_select_missing_button =",
            "self.tools_download_button =", "self.tools_delete_button =", "self.tools_row_widgets =",
            "self.python_env_status =", "self.python_env_hint =", "self.python_env_status_icon_label =",
            "self.python_env_status_label =", "self.tool_image_statuses =", "self.tool_image_sizes =",
            "self.tool_image_installed_sizes =", "self.tools_hub_size_loading =", "self.tool_status_labels ="
        ]):
            continue
...
for old, new in replacements.items():
    if old.startswith("self."):
        attr = old.split(".")[1]
        main_content = main_content.replace(f"self.{attr}", f"self.tools_ctrl.{new.split('.')[1]}")
...
for old, new in replacements.items():
    if old.startswith("self."):
        attr = old.split(".")[1]
        tools_tab_content = tools_tab_content.replace(f"ctrl.{attr}", f"ctrl.{new.split('.')[1]}")
